namespace KeyifyClassLibrary.Service_Models
{
    public static class Degree
    {
        public static string First = "1";
        public static string FlatSecond = "b2";
        public static string Second = "2";
        public static string SharpSecond = "#2";
        public static string FlatThird = "b3";
        public static string Third = "3";
        public static string FlatFourth = "b4";
        public static string Fourth = "4";
        public static string SharpFourth = "#4";
        public static string FlatFifth = "b5";
        public static string Fifth = "5";
        public static string SharpFifth = "#5";
        public static string FlatSixth = "b6";
        public static string Sixth = "6";
        public static string SharpSixth = "#6";
        public static string FlatSeventh = "b7";
        public static string FlatFlatSeventh = "bb7";
        public static string Seventh = "7";
        public static string SharpSeventh = "#7";
        public static string Eighth = "8";
    }
}
